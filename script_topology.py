...
def write_to_csv(filename, data):
    with open(filename, 'w', newline='') as csvfile:
        ...
        writer.writerow(['num_nodes', 'avg_tdt', 'avg_ldt', 'avg_total','avg_lldp_len','avg_pkt_len','avg_lldp_count','avg_pkt_count'])
...
if __name__ == '__main__':
    ...
    while running and (get_target(args.topology,i,'agg') < args.maxsize):
        ldt_sum, tdt_sum, lldp_sum, lldp_sum_count, pkt_sum, pkt_sum_count = 0,0,0,0,0,0
        ...
        for j in range(0, args.trials):
            ...
            with open('output/topo_disc_'+args.controller_name+'.txt', 'r') as f:
                ...
                if(len(values) > 1):
                    topology_discovery_time, total_discovery_time, total_lldp, count_lldp, total_pkt, count_pkt = float(values[0]), float(values[1]), float(values[2]), float(values[3]), float(values[4]), float(values[5])
        ...
        avg_tdt, avg_ldt, avg_lldp, avg_lldp_count, avg_pkt, avg_pkt_count = (tdt_sum / args.trials), (ldt_sum/args.trials), (lldp_sum/args.trials), (lldp_sum_count/args.trials), (pkt_sum/args.trials), (pkt_sum_count/args.trials)
        data.append([target_length, avg_tdt, avg_ldt, (avg_tdt+avg_ldt),avg_lldp, avg_pkt, avg_lldp_count, avg_pkt_count])
